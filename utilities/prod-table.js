...
//get productions without videos
...
//convert years to numbers and get production medium
const prodTable = [];
for (const p of info){
...
//note where each production is located (website, excel sheet, both)
...
const sheetName = prompt('Enter desired Excel sheet name: ')
...
//save as json file for passing into visualizations
...
const filename = prompt('Enter desired JSON file name: ');
writeFileSync(`./${filename}.json`, JSON.stringify(finalProdTableJson, null, 4));
...
/**
 * Checks if productions are the same
 */
function sameProd(prodA, prodB){
    return prodA.Title === prodB.Title && prodA.Director === prodB.Director && prodA.Year === prodB.Year;
}
